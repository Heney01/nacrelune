...
export async function deleteModel(
  ...
  'use server';
  try {
    console.log(`--- deleteModel action for ${jewelryTypeId}/${modelId} ---`);
    ...
    console.log(`Successfully deleted model ${modelId} from ${jewelryTypeId}`);
    revalidatePath('/', 'layout');
    revalidatePath('/admin/dashboard');
    return { success: true, message: 'Modèle supprimé avec succès.' };
  } catch (error: any) {
    console.error(`Error deleting model ${modelId} from ${jewelryTypeId}:`, error);
