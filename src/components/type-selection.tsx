...
interface TypeSelectionProps {
    jewelryTypes: Omit<JewelryType, 'models'>[];
    onTypeSelect: (type: Omit<JewelryType, 'models'>) => void;
}

export function TypeSelection({ jewelryTypes, onTypeSelect }: TypeSelectionProps) {
        ...
        <section className="text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {jewelryTypes.map((type) => (
                    <Card key={type.id} className="cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-transform duration-300" onClick={() => onTypeSelect(type)}>
